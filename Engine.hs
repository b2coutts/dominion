...
actPhase game@(Game crds amts usrs (Turn usr buys gold acts) _)
    | acts == 0 = return game
    | null $ catMaybes $ map (func . (crds M.!)) hand =  return game
    | otherwise = do
...
-- simulates the entire game, eventually producing the winner
simGame :: Game -> IO String
simGame game@(Game crds amts usrs trn _)
    | isOver game = let (usri, vp) = getWinner game
        ...
        oPrint ig $ printf "%s won the game with %d victory points!\n"
                           (name $ usrs !! usri) vp
        return $ name $ usrs !! usri
    | otherwise = do
        game'@Game{turn=Turn usr _ gld _} <- actPhase game
        let endGold = sum $ map (valu . (crds M.!)) $ hand $ actor game
        game'' <- buyPhase game'{turn=trn{gold = gld + endGold}}
        simGame game''{turn = Turn (mod (usr+1) $ length usrs) 1 0 1}
